...
let conteinerProdutos = document.querySelector(".produto"); 
...
function mostrarProdutos() {
    htmlProdutos = "";
    let produtosFiltrados = produtos.filter(pdr => {

        let passouCategoria = categoriaAtiva == "all" || pdr.categoria == categoriaAtiva;

        let passouPesquisa = pdr.nome.toLowerCase().includes(textpesquisa.toLowerCase()); 
               return passouPesquisa && passouCategoria;
    });
    
    
    produtosFiltrados.forEach(pdr => {
        
        htmlProdutos = htmlProdutos + `
            <div class="card">
                <img class="card-img" src="${pdr.imagem}" alt="${pdr.nome}">
                <div class="informação">
                    <h3 class="informação-titulo">${pdr.nome}</h3>
                    <p class="informação-descrição">${pdr.descricao}</p>
                    <p class="preco"> R$ ${pdr.preco}</p>
...
function pesquisar() {
    textpesquisa = inputPesquisa.value;
    mostrarProdutos();
}
